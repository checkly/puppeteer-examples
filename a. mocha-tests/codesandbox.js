...
describe('codesandbox.io', () => {
  it('creates a vanilla.js sandbox', async () => {
    ...
    await page.screenshot({ path: 'mocha-codesandbox-1.png'})

    await page.click('a[href="/s"]')
    // await page.waitForSelector('a[href="/s/vue"]')
    await page.waitForSelector('button div.sc-gZMcBi')
    await page.screenshot({ path: 'mocha-codesandbox-2.png'})

    // await page.click('a[href="/s/vue"]')
    await page.click('button div.sc-gZMcBi')
    await page.waitForSelector('.react-monaco-editor-container')
    const editor = await page.$('.react-monaco-editor-container')
    await page.screenshot({ path: 'mocha-codesandbox-done.png' })
    assert.ok(editor)
  }).timeout(30000)
})
